(() => {
  ...
  const categoryMap = {
    'AI Foundations': { icon: 'blocks', scene: 'Byggklossar', hook: 'Se detta som en grundkloss som andra AI-delar staplas ovanpå.' },
    'LLM Mechanics': { icon: 'tokens', scene: 'Ordmaskin', hook: 'Föreställ dig små ord-bitar som åker genom en maskin och blir ett svar.' },
    'Prompting': { icon: 'prompt', scene: 'Instruktionskort', hook: 'Tänk att du skriver en tydlig beställning till en smart hjälpare.' },
    'RAG & Knowledge': { icon: 'library', scene: 'Bibliotek', hook: 'AI:n går till rätt bokhylla, hämtar fakta och svarar med stöd.' },
    'Agents & Automation': { icon: 'robot', scene: 'Robot med verktyg', hook: 'En hjälparrobot tar ett steg, använder ett verktyg, kollar resultatet och fortsätter.' },
    'Evaluation': { icon: 'checklist', scene: 'Rättningsmall', hook: 'Som att rätta en läxa: svaret måste jämföras mot kriterier.' },
    'Production & System Design': { icon: 'pipeline', scene: 'Fabrikslinje', hook: 'AI-systemet är en kedja av steg som måste fungera varje dag.' },
    'Safety & Governance': { icon: 'shield', scene: 'Sköld och regler', hook: 'Regler och skydd håller AI:n från att göra fel saker.' },
    'Tools & Ecosystem': { icon: 'toolbox', scene: 'Verktygslåda', hook: 'Varje verktyg har ett jobb, precis som hammare, såg och skruvmejsel.' },
    'Data Engineering': { icon: 'database', scene: 'Dataskafferi', hook: 'Data är ingredienserna; ordning på ingredienserna gör AI:n bättre.' },
    'Math & ML Essentials': { icon: 'gauge', scene: 'Poängmätare', hook: 'Mätaren visar om modellen gissar bättre eller sämre.' },
    'Multimodal AI': { icon: 'eye', scene: 'Ögon och öron', hook: 'AI:n får fler sinnen än text: bild, ljud, video eller dokument.' },
    'AI Expert Meta-Skills': { icon: 'map', scene: 'Strategikarta', hook: 'Du väljer rätt väg, rätt verktyg och rätt kontroll för problemet.' }
  };
  ...
  function visualFor(type, card) {
    ...
    if (type === 'checklist') return `
      <svg viewBox="0 0 420 220" role="img" aria-label="Evaluation checklist visual">
        ...
        <text x="210" y="208" text-anchor="middle" fill="#e5edf7" font-size="16" font-weight="800">kriterier → test → förbättra</text>
      </svg>`;
    ...
    if (type === 'pipeline') return `
      <svg viewBox="0 0 420 220" role="img" aria-label="Production pipeline visual">
        ...
        <text x="210" y="195" text-anchor="middle" fill="#e5edf7" font-size="16" font-weight="800">bygg för drift: loggar, fallbacks, latency</text>
      </svg>`;
    if (type === 'toolbox') return `
      <svg viewBox="0 0 420 220" role="img" aria-label="Toolbox visual">
        ...
        <text x="210" y="205" text-anchor="middle" fill="#e5edf7" font-size="16" font-weight="800">välj rätt verktyg för rätt jobb</text>
      </svg>`;
    if (type === 'tokens') return `
      <svg viewBox="0 0 420 220" role="img" aria-label="Token flow visual">
        ...
        <text x="210" y="195" text-anchor="middle" fill="#e5edf7" font-size="16" font-weight="800">text blir tokens, tokens blir nästa token</text>
      </svg>`;
    if (type === 'prompt') return `
      <svg viewBox="0 0 420 220" role="img" aria-label="Prompt instruction visual">
        ...
        <text x="210" y="204" text-anchor="middle" fill="#e5edf7" font-size="16" font-weight="800">mål + kontext + format + kvalitet</text>
      </svg>`;
    if (type === 'database') return `
      <svg viewBox="0 0 420 220" role="img" aria-label="Database visual">
        ...
        <text x="210" y="203" text-anchor="middle" fill="#e5edf7" font-size="16" font-weight="800">ren data → bättre AI-beslut</text>
      </svg>`;
    if (type === 'eye') return `
      <svg viewBox="0 0 420 220" role="img" aria-label="Multimodal visual">
        ...
        <text x="210" y="205" text-anchor="middle" fill="#e5edf7" font-size="16" font-weight="800">se, läs, lyssna, verifiera</text>
      </svg>`;
    if (type === 'gauge') return `
      <svg viewBox="0 0 420 220" role="img" aria-label="Gauge visual">
        ...
        <text x="210" y="190" text-anchor="middle" fill="#e5edf7" font-size="16" font-weight="800">mät fel, förbättra modellen</text>
      </svg>`;
    if (type === 'map') return `
      <svg viewBox="0 0 420 220" role="img" aria-label="Strategy map visual">
        ...
        <text x="210" y="207" text-anchor="middle" fill="#e5edf7" font-size="16" font-weight="800">problem → val → risk → resultat</text>
      </svg>`;
    return `
      <svg viewBox="0 0 420 220" role="img" aria-label="Concept blocks visual">
        ...
        <text x="210" y="195" text-anchor="middle" fill="#e5edf7" font-size="16" font-weight="800">${label} kopplas till ${cat}</text>
      </svg>`;
  }

  function makeVisualPrompt(card, meta) {
    const connections = (card.connections || []).slice(0, 3).join(' → ') || card.category;
    return `Rita ${card.title} som: ${meta.scene}. Lägg till tre etiketter: ${connections}. Säg högt varför bilden passar begreppet.`;
  }

  function makeAudioText(card, meta) {
    const simple = `${card.title}. Tänk på bilden: ${meta.scene}. ${meta.hook}`;
    const full = `${card.title}. ${card.back}. ${card.example ? 'Exempel: ' + card.example : ''}`;
    return { simple, full };
  }
    ...
    document.querySelectorAll('.answer-reveal:not([data-dual-coded])').forEach((box) => {
      ...
      box.insertAdjacentHTML('beforeend', `
        <div class="dual-code-panel" data-card-title="${escapeHTML(card.title)}">
          <h3>Visuell minnesbild</h3>
          <div class="dual-code-grid">
            <div class="dual-code-svg">${visualFor(meta.icon, card)}</div>
            <div class="dual-code-copy">
              <p><strong>Minneskrok:</strong> ${escapeHTML(meta.hook)}</p>
              <p><strong>Rita själv:</strong> ${escapeHTML(makeVisualPrompt(card, meta))}</p>
              <p><strong>Snabb koppling:</strong> ${escapeHTML(card.title)} → ${escapeHTML((card.connections || []).slice(0, 3).join(' → ') || card.category)}</p>
              <div class="dual-code-actions">
                <button class="btn ghost" data-audio="simple" data-title="${escapeHTML(card.title)}" data-text="${escapeHTML(audio.simple)}">Spela enkel ljudförklaring</button>
                <button class="btn ghost" data-audio="full" data-title="${escapeHTML(card.title)}" data-text="${escapeHTML(audio.full)}">Spela längre ljudförklaring</button>
                <button class="btn ghost" data-audio="cue" data-category="${escapeHTML(card.category)}">Ljudsignal</button>
                <button class="btn ghost" data-audio="stop">Stoppa ljud</button>
              </div>
            </div>
          </div>
        </div>
  ...
  function speak(text, mode = 'full') {
    if (!('speechSynthesis' in window)) {
      alert('Din webbläsare stöder inte text-till-tal här.');
      return;
    }
    ...
    utterance.voice = sv || en || null;
    utterance.lang = sv ? 'sv-SE' : 'en-US';
